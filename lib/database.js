...
function save_tx(txid, cb) {
  ...
  lib.get_rawtransaction(txid, function(tx){
    if (tx != 'There was an error. Check your console.') {
      lib.get_block(tx.blockhash, 1, function(block){
        if (block) {
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                    var newTx = new Tx({
                      ...
                      txcomment: tx['floData'],
                      txcomment_exists: ( (tx['floData'] != null) && (tx['floData'] != '') && (tx['floData'] != "") ),
                    });
...
module.exports = {
  ...
  get_last_blocks: function(count, min, cb) {
    Tx.find().sort({"blockindex":-1}).limit(1).exec(function (err, blockmax){
      if (err) {
        return cb(err);
      }
      else {
        // Find the latest count blocks from the database.
	console.log(blockmax);
	console.log(blockmax[0].blockindex);
        Tx.aggregate([ {"$match": {"blockindex": {"$gt": blockmax[0].blockindex-count}}}, {"$group": {"_id": {"blockindex":"$blockindex", "blockhash":"$blockhash", "timestamp":"$timestamp"}}}, {"$sort": {"_id.blockindex":-1}}]).exec(function(err, blocks) {
          if (err) {
		console.log("ERROR");
			console.log(err);
            return cb(err);
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({'txcomment_exists': true}).sort({blockindex:-1}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        //console.log(txs);
                  ...
                  lib.get_nextin( function (nextin) {
                    lib.syncLoop(count, function (loop) {
                      ...
                      lib.get_blockhash(height-i, function (hash) {
                        lib.get_block(hash, 1, function (block) {
